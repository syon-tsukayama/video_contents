...
SELECT `id`, `title`, `content`, `image_name`, `mp4_file_name`, `ogv_file_name`
FROM `contents_table`
WHERE `publish_status` = 1 AND `id` NOT IN ({$bind_param})
LIMIT 6;
EOS;

$stmt_select_contents_notice = $conn->prepare($sql);

foreach($displayed_content_ids as $key => $content_id)
{
    $stmt_select_contents_notice->bindValue(':content_id_'.$key, $content_id, PDO::PARAM_INT);
}
$stmt_select_contents_notice->execute();

$notice_contents = array();
// 検索結果取得
while($row = $stmt_select_contents_notice->fetch())
{
    $notice_contents[] = $row;
}
?>
        ...
        <div class="span8 well well-small">
            ...
            <div class="row-fluid">
                ...
                if(!empty($notice_contents)):
                    $count = 0;
                    foreach($notice_contents as $row):
                ...
                <?php if(($count % 2) === 0 || $count == count($notice_contents)): ?>
                </ul>
                <?php endif; ?>
